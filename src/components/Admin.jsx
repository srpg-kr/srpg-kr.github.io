...
import { loginWithGitHub, handleGitHubRedirect, getGitHubToken, logout, getIssues, rejectSuggestion, acceptSuggestion, parseIssueBody, dispatchWorkflow, checkAdminPermissions } from '../services/github';
  ...
  useEffect(() => {
    const handleAuth = async () => {
      ...
      if (existingToken) {
        ...
        const adminStatus = await checkAdminPermissions(existingToken);
        setIsAdmin(adminStatus);
      } else {
        const new_token = await handleGitHubRedirect();
        if (new_token) {
          setToken(new_token);
          const adminStatus = await checkAdminPermissions(new_token);
          setIsAdmin(adminStatus);
        }
      }
    };
    ...
    <div style={{ padding: '1rem' }}>
      ...
      {token ? (
        isAdmin ? (
          ...
          <div>
            <p>Welcome, you are logged in, but you do not have administrator privileges for this repository.</p>
            <button onClick={handleLogout}>Logout</button>
          </div>
        )
      ) : (
